...
4. **生成实体设计文档 (Output Design Document)**：将上述内容整合生成一份标准 Markdown 格式的《技术契约与架构设计书》。**必须将其作为实体文件写入到项目 `.ai/specs/` 目录下（如 `feature-name-spec.md`）**。内容必须明确区分为：数据模型层(Schema) -> 后端服务层(API) -> 前端交互层(View) 三个独立章节的改动意图及受波及文件清单。
